GCCHCoin-qt: Qt4 GUI for GCCHCoin
===============================
...
Alternatively, install Qt Creator and open the `GCCHCoin-qt.pro` file.

An executable named `GCCHCoin-qt` will be built.
...
A warning for people using the *static binary* version of GCCHCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of GCCHCoin is linked against libdb5.3.
...
installed causes GCCHCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
