...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #GCCHCoinTEST2\r");
            Send(hSocket, "WHO #GCCHCoinTEST2\r");
        } else {
            // randomly join #GCCHCoin00-#GCCHCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #GCCHCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GCCHCoin%02d\r", channel_number).c_str());
        }
